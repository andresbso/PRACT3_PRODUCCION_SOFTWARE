...
def test_remove_expense_reduces_total():
    ...
    service.create_expense(title="Libro", amount=6, description="")
    service.create_expense(title="Revista", amount=2, description="")

    service.remove_expense(1)
    service_list = service.list_expenses()

    assert len(service_list) == 1
    assert service_list[0].title == "Revista"
...
def test_update_expense_partial_fields():
    ...
    assert service_list[0].title == "Camiseta"
    assert service_list[0].amount == 18
    assert service_list[0].description == "Ropa"
...
def test_total_amount_after_removal():
    ...
    amount1 = service.total_amount()

    service.remove_expense(1)

    amount2 = service.total_amount()
